...
public class GateView extends FixedPanel implements ItemListener, MouseListener {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (gate.getOutputSize() == 2) {
            ...
            g.drawImage(image, BORDER + SWITCH_SIZE, 1, GATE_WIDTH, GATE_HEIGHT, this);
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        int y = GATE_HEIGHT / 2;
        int x2 = BORDER + SWITCH_SIZE + GATE_WIDTH + LIGHT_SIZE / 2;
        int y2 = GATE_HEIGHT / (gate.getOutputSize()) - (SWITCH_SIZE / 2);
        ...
        if (gate.getOutputSize() == 2) {
            int x3 = BORDER + SWITCH_SIZE + GATE_WIDTH + LIGHT_SIZE / 2;
            int y3 = GATE_HEIGHT / (gate.getOutputSize() - 1) - (SWITCH_SIZE);

            if (Math.sqrt(Math.pow(x3 - e.getX(), 2) + Math.pow(y3 - e.getY(), 2)) < LIGHT_SIZE / 2) {
                Color color = JColorChooser.showDialog(this, null, this.color);
            ...
            if (Math.sqrt(Math.pow(x2 - e.getX(), 2) + Math.pow(y2 - e.getY(), 2)) < LIGHT_SIZE / 2) {
